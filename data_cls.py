...
class MotorData(Dataset):
    def __init__(self, root, split='train', num_points=2048, test_area='Validation',
        ...
        for idx in range(len(num_points_all)):
            ...
            self.motors_idxs.extend(motor_index_onemotor)
    
    def __len__(self):
        return len(self.motors_idxs)
    
    def __getitem__(self, index):
        point_set = self.all_points[self.motors_idxs[index]]
        # labels = self.all_labels[self.motors_index[index]]
        types = self.all_types[self.motors_idxs[index]]
        n_points = point_set.shape[0]
...
if __name__ == '__main__':
    import torch
    data = MotorData(root='E:\\dataset', split='train', test_area='Validation')
